            ...
            <v-dialog v-model="dialog"
                ...
                <v-form>
                    <v-container style="background-color: #FFFFFF;">
                        ...
                        <v-row>
                            <v-col cols="6">
                                <v-text-field :value=popup.thesisType
                                              label="Thesis degree"></v-text-field>
                            </v-col>
                            <v-col cols="6">
                                <v-text-field :value=popup.studyType
                                              label="Study type"></v-text-field>
                            </v-col>
                        </v-row>
                        <v-row>
                            <v-col cols="6" v-for="student in popup.students">
                        ...
                        <v-row>
                            <v-col cols="12" class="text-center">
                                <v-btn style="background-color: green; width: 200px; height: 50px; font-size: 24px;" class="mx-12"
                                       text
                                       color="#FFFFFF"
                                       @click="dialog = false">
                                    Save
                                </v-btn>
                                <v-btn style="background-color: red; width: 200px; height: 50px; font-size: 24px;" class="mx-12"
                                       text
                                       color="#FFFFFF"
                                       @click="dialog = false">
                                    Cancel
            ...
            <v-col cols="12">
                <v-data-table :headers="headers"
                    ...
                    <template v-slot:item.thesisType="{ item }">
                        <span style="float: left; font-size: 24px; font-weight: bold;">{{ item.thesisType }}</span>
                    </template>
                    <template v-slot:item.studyType="{ item }">
                        <span style="float: left; font-size: 24px; font-weight: bold;">{{ item.studyType }}</span>
                    </template>
                    <template v-slot:item.freeSlots="{ item }">
                        <span style="float: left; font-size: 24px; font-weight: bold;">{{ item.maxStudents - item.freeSlots }} / {{ item.maxStudents}} </span>
                    </template>

                    <template v-slot:body.append>
                        <td :key="index"  @click="dialog=true;">
                            <span style="font-size: 24px; font-weight: bold;"><v-icon style="margin-bottom: 6px;">mdi-plus-box</v-icon> Add a new thesis</span>
                        </td>
                    </template>
