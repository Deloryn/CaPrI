	...
	<v-app id="app" style="background-color: #EEEEEE;">
		<div v-if="$route.path !== '/'">
			<navBar>
				<div slot="navItems">
					<navStudentItems
						v-if="parsedToken.userType === UserTypes.student"
					></navStudentItems>
					<navPromoterItems
						v-if="parsedToken.userType === UserTypes.promoter"
					></navPromoterItems>
					<navDeanItems v-if="parsedToken.userType === UserTypes.dean"></navDeanItems>
				</div>
			</navBar>
			<topBar> </topBar>
		</div>
